...
https://github.com/OALcementys/codebrim_challenge
"""
...
def build_vistra_1(cp_path, img_size=224, num_cls=6, quantized=False):
    if quantized:
    ...
    if quantized:
        ...
        msg = model.load_state_dict(state_dict, strict=False)
        print('Pretrained weights found at {} and loaded with msg: {}'.format(cp_path, msg))
    
    # Print a quick summary
    model_summary = 'The model was instantiated from {} with the following arguments:\n'.format(cp_path)
    model_summary += "img_size: {}\n".format(state_dict['img_size'])
    model_summary += "num_class: {}\n".format(state_dict['num_class'])
    model_summary += "cat_to_name: {}\n".format(state_dict['cat_to_name'])
    model_summary += "dataset: {}\n".format(state_dict['dataset'])
    print('=====Model summary=====')
    print(model_summary)
    return model, state_dict['cat_to_name']
...
if __name__ == '__main__':
    ...
    # cp = {}
    # cp['orig_cp'] = torch.load(cp_path, map_location="cpu")
